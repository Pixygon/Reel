...
// REEL_PLUGIN_SLOT — replaced by the active effect plugin's source; same
// contract as video.wgsl. Keep on one line.
...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // The effect plugin: after the grade, before the trims — identity
    // unless a plugin pipeline is bound. Same contract as video.wgsl.
